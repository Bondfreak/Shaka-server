...
class CoreClient:
    def __init__(self, base_url: str, *, timeout_seconds: float = 20.0, retry_delay_seconds: float = 5.0, retry_count: int = 3) -> None:
        if retry_count < 0:
    ...
    def _request(self, path: str) -> httpx.Response:
        return httpx.get(f"{self._base_url}{path}", timeout=self._timeout, follow_redirects=False)
    ...
    def _get(self, path: str) -> CoreResult:
        ...
        for attempt in range(self._retry_count + 1):
            ...
            if attempt < self._retry_count and (last_transport_error is not None or transient_status):
    ...
    def resolve_asset_instance(self, context_public_id: str, asset_public_id: str) -> CoreResult:
        return self._get(f"/api/v1/asset-instances/{context_public_id}/resolve-asset/{asset_public_id}")
    ...
    def cog_topology(self, system_id: str) -> CoreResult:
        return self._get(f"/api/v1/cog/topologies/{system_id}")

    def cog_diagnostic(self, scenario_id: str) -> CoreResult:
        return self._get(f"/api/v1/cog/diagnostics/{scenario_id}")
